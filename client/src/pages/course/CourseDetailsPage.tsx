...
const CourseDetailsPage = () => {
  ...
  async function fetchCourse() {
    ...
    try {
      const link =
        import.meta.env.VITE_SERVER_HOST + `courses/${params.courseID}`;
      const response = await fetch(link, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
  ...
  const updateCourseHandle = () => {
    ...
    const updateCourse = async () => {
      try {
        dispatch(setLoadingContentState(true));
        await fetch(import.meta.env.VITE_SERVER_HOST + "courses/" + oldID, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            authorization: authHeader(),
  ...
  if (isEditing) {
    return (
      <>
        {isEditing &&
        ((auth() ? auth()!.role : null) === "admin" ||
          (auth() ? auth()!.role : null) === "garant") ? (
          <Fab
            variant="extended"
            ...
            <div className="list-pages-list-container">
              ...
              <ul>
                ...
                {/* {activities.map((activity: Activity) => (
                  <li key={activity.id} className="list-item-properties">
                    <span className="list-item-property">{activity.type}</span>
                    <span className="list-item-property">
                      {activity.duration}
                    </span>
                    <span className="list-item-property user-role">
                      {activity.recurrence}
                    </span>
                  </li>
                ))} */}
            ...
            <div className="list-pages-list-container">
              ...
              <ul>
                <li className="list-header">
                  ...
                  <span className="header-item">čas začátku</span>
                  <span className="header-item">doba trvání</span>
                  <span className="header-item">místnost</span>
                  <span className="header-item">vyučující</span>
                </li>
                {course.instances.map((instance: Instance) => (
                  <li key={instance.id} className="list-item-properties">
